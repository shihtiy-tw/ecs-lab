...
data "aws_ami" "ecs_optimized" {
  ...
  filter {
    name   = "name"
    values = ["amzn2-ami-ecs-kernel-*gpu*-x86_64-ebs"]
    # amzn2-ami-ecs-kernel-5.10-gpu-hvm-2.0.20241120-x86_64-ebs
  }
...
resource "aws_iam_instance_profile" "ecs_container_instance_profile" {
  name = "${var.cluster_name}-gpu-ecs-agent"
...
resource "aws_security_group" "instance_sg" {
  name        = "${var.container_instance_name}-security-group"
